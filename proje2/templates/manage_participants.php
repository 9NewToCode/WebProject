...
<main style="padding: 20px;">
    ...
    <form action="manage_participants?id=<?= $data['aid'] ?>" method="post">
        <div style="width: 20%;background: gray; margin-left: auto; margin-top: 10px; padding: 4px 8px; border-radius: 4px; border-color: black; border-width: 2px;
                    display: flex; flex-direction: column;">
            <div>
                <span style="font-size: large; padding: 4px 8px;">กรอก UID ที่ต้องการเช็ค</span>
                <input style="width: 50%; font-size: large; margin: 4px 8px;"
                type="number" name="target_uid">
            </div>
            <span style="font-size: large; padding: 4px 8px;">กรอก OTP ที่ต้องการเช็ค</span>
            <div style="width: 50%;">
                <input style="width: 100%; font-size: large; margin: 4px 8px;" name="otp" id="otp" type="text">
                <button style="width: 100%; font-size: large; margin: 4px 8px; background: white">เช็ค</button>
            </div>
        </div>
    </form>

    <table style="width: 100%; margin-top: 20px; border-collapse: collapse; text-align: left; border-color: black; border-width: 1px;">
        <thead style="background: #f4f4f4;">
            <tr>
            <th style="padding: 10px;">ชื่อผู้ใช้</th>
            <th style="padding: 10px;">อีเมล</th>
            <th style="padding: 10px;">สถานะ</th>
            <th style="padding: 10px;">สถานะการเข้างาน</th>
        ...
        <tbody>
            <?php while ($p = $data['participants']->fetch_assoc()): ?>
                <tr>
                    ...
                    <td style="padding: 10px;">
                        <span style="padding: 4px 8px; border-radius: 4px; background:
                        <?= $p['Chk_In_Status'] == 'Checked' ? '#d4edda' :
                        ($p['Chk_In_Status'] == 'Unchecked' ? '#f8d7da' : '#fff3cd') ?>;">
                            <?= $p['Chk_In_Status'] ?>
                        </span>
                    </td>
                    <td style="padding: 10px;">
