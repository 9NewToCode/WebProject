...
<main style="padding: 20px; max-width: 800px; margin: auto;">
    ...
    <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px;">
        <?php if (!empty($data['images'])): ?>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <p style="color: #999;">ยังไม่มีรูปภาพกิจกรรม</p>
        <?php endif; ?>
    </div>
    ...
    <div style="margin-top: 30px; display: flex; flex-wrap: wrap; gap: 10px; align-items: center;">
        ...
        <?php if ($current_uid > 0): ?>
            <?php if ($current_uid === $act['CID'] || $user_role === 'admin'): ?>
                <a href="/activity_stats?id=<?= $act['AID'] ?>" style="padding: 10px 20px; background: #6f42c1; color: white; text-decoration: none; border-radius: 5px; font-weight: bold;"> ดูสถิติ</a>
